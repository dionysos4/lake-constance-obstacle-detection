...
class ImageLidarEvaluation():
    ...
    def __get_point_clusters(self, pc_hom, bounding_boxes, img_height, img_width, T_cam_velo):
        # use only points in front of the sensor
        mask = pc_hom[:,0] > 4
        pc_hom = pc_hom[mask]

        R_l_hom = utils.create_T_matrix(self.R_l, np.zeros((3)))
        img_coords_homo = (self.K_l @ np.linalg.inv(R_l_hom)[:3] @ T_cam_velo @ np.expand_dims(pc_hom, axis=2)).squeeze(2)
        img_coords = np.true_divide(img_coords_homo[:,:2], img_coords_homo[:,[-1]])

        # plot just points in camera frustum
        height = img_height
        width = img_width
        ...
        pointcloud_in_img = pc_hom[mask]

        point_clusters = []
        for box in bounding_boxes:
            cluster_mask = (img_coords[0] >= box[0]) &\
                        (img_coords[0] <= box[2]) &\
                        (img_coords[1] >= box[1]) &\
                        (img_coords[1] <= box[3])
            cluster_pc = pointcloud_in_img[cluster_mask]
            point_clusters.append(cluster_pc)

        ### only for image lidar fusion
        ### if no points in point cluster, no valid detection FP and remove it
        index_list = []
        for c in range(len(point_clusters)):
            if len(point_clusters[c]) == 0:
                index_list.append(c)
        new_point_clusters = []
        for c in range(len(point_clusters)):
            if c in index_list:
                ...
                new_point_clusters.append(point_clusters[c])

        return new_point_clusters
    

    def __get_3d_boxes(self, point_clusters, T_plane_cam, T_cam_velo):
        detected_boxes = []
        for clustered_pointcloud in point_clusters:
            clustered_pointcloud_hom = (T_plane_cam @ T_cam_velo @ np.expand_dims(clustered_pointcloud, axis=2)).squeeze(axis=2)
    ...
    def estimate(self):
        for hdf5_file in tqdm(sorted(os.listdir(self.dataset_path))):
            ...
            # find pointclusters
            point_clusters = self.__get_point_clusters(pc_hom, bounding_boxes, left_img_undistorted.shape[0], left_img_undistorted.shape[1], T_cam_velo)
            # compute bounding box
            detected_boxes = self.__get_3d_boxes(point_clusters, T_plane_cam, T_cam_velo)
...
# model.eval()
# eval = ImageLidarEvaluation("/media/dennis/3B9FC6C559F7A944/converted_object_detection_dataset/test", model, metric, 0.8)
