...
from metric.detection.helpers import _fix_empty_tensors, _input_validator
...
class MeanAveragePrecision(Metric):
    ...
    Raises:
        ...
        ValueError:
            If ``iou_type`` is not one of ``"bbox"``
        ValueError:
    ...
    def _get_coco_format(
        ...
        for image_id, (image_boxes, image_labels) in enumerate(zip(boxes, labels)):
            ...
            for k, (image_box, image_label) in enumerate(zip(image_boxes, image_labels)):
                if self.iou_type == "bbox" and len(image_box) != 8:
                    raise ValueError(
